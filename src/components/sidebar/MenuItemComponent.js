...
const useStyles = createUseStyles({
    ...
    leftBar: {
        borderLeft: ({ theme, level }) =>
            level > 1 ? 'none' : `3px solid ${theme.color.DashFont}`
    },
    title: {
        fontSize: 18,
        lineHeight: '20px',
        letterSpacing: '0.2px',
        color: ({ theme, isActive }) => (isActive ? theme.color.darkgreen : theme.color.black),
        marginLeft: 24
    }
});
...
function MenuItemComponent({ children, icon: Icon, id, items = [], level = 1, onClick, title }) {
    ...
    const iconColor = isActive ? theme.color.black : theme.color.icon;
